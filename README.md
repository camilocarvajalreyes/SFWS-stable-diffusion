...
After [obtaining the `stable-diffusion-v1-*-original` weights](#weights), download them and put them on the following (new) directory:
```
mkdir -p models/ldm/stable-diffusion-v1/
```
and sample with
```
python scripts/txt2img.py --ckpt models/ldm/stable-diffusion-v1/sd-v1-4.ckpt --prompt "A shirt of composer Sergei Rachmaninov as if it were a Heavy Metal band shirt" --ddim_steps 50 --gamma 0 --clf "None" --n_samples 3 --n_iter 2 --seed 42
```
...
usage: txt2img.py [-h] [--prompt [PROMPT]] [--outdir [OUTDIR]] [--skip_grid] [--skip_save] [--ddim_steps DDIM_STEPS] [--plms] [--laion400m] [--fixed_code] [--ddim_eta DDIM_ETA]
                  ...
                  [--seed SEED] [--precision {full,autocast}] [--gamma GAMMA] [--clf {CLIP_concept,CLIP_matching,ALIGN_matching,None}] [--threshold_clf THRESHOLD_CLF] [--concept CONCEPT]

added arguments:
  --gamma GAMMA         Unguidance strength (Guidance factor in Universal Guidance)
  --clf {CLIP_concept,CLIP_matching,ALIGN_matching,None}
                        classifier for unguidance, 'None' for no unguidance
  --threshold_clf THRESHOLD_CLF
                        threshold for applying the classifier unguidance. If the probability is under the threshold, the noise network remains unchanged.
  --concept CONCEPT     Concepts to include in CLIP_concepts classifier, separated by a comma
